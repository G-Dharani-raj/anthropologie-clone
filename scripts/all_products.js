...
const init_load = async () => {
	...
	let res = await fetch(`https://mock-api-4u7p.onrender.com/${keyword}`);
...
const sort_fn = async (value) => {
	...
	let res = await fetch(
		`https://mock-api-4u7p.onrender.com/${current_selection}?_sort=price&_order=${value}`
	);
	let data = await res.json();
	append(data);
};
...
function signinnumber() {
	...
	userdata.forEach((e) => {
		if (e.mobile === 0) {
			...
			document.getElementById("enternumbersignin").style.borderColor =
				"red";
		} else if (
			document.getElementById("enternumbersignin").value == e.mobile
		) {
...
function signinbuttonemail() {
	...
	userdata.forEach((e) => {
		if (
			document.getElementById("emailsigninpassword").value == e.password
		) {
			...
			document.getElementById("emailsigninpassword").style.borderColor =
				"red";
			document.getElementById("emailsigninpassword").placeholder =
...
if (success == "successfull") {
	...
	document.getElementById("signin").innerText = JSON.parse(

...
if (localStorage.getItem("admin_stay_logged") == "false") {
	...
	async function getDataHeroku() {
		try {
			let res = await fetch("https://mock-api-4u7p.onrender.com/shoes");
	...
	async function deleteProduct(element) {
		...
		try {
			let res = await fetch(
				`https://mock-api-4u7p.onrender.com/shoes/${deleteID}`,
				{
					method: "DELETE",
					headers: {
						"Content-Type": "application/json",
	...
	async function updatePrice(element) {
		try {
			...
			let res = await fetch(
				`https://mock-api-4u7p.onrender.com/shoes/${prod_id}`,
				{
					method: "PATCH",

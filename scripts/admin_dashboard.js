...
if (localStorage.getItem("admin_stay_logged") == "false") {
	document.getElementById("admin_dashboard_container").innerHTML = "";
	document.getElementById("admin_dashboard_container").classList =
		"notlogged";
	...
	async function getDataHeroku() {
		try {
			let res = await fetch(
				"https://mock-api-4u7p.onrender.com/products?category=dress&category=clothing"
			);
	...
	async function addProd() {
		...
		try {
			let res = await fetch(
				`https://mock-api-4u7p.onrender.com/${category}/`,
				{
					method: "POST",
			...
			let ressponse = await fetch(
				`https://mock-api-4u7p.onrender.com/products/`,
				{
					method: "POST",
					body: JSON.stringify(newData),
					headers: {
	...
	async function deleteProduct(element) {
		...
		try {
			let res = await fetch(
				`https://mock-api-4u7p.onrender.com/products/${deleteID}`,
				{
					method: "DELETE",
					headers: {
						"Content-Type": "application/json",
	...
	async function updatePrice(element) {
		try {
			...
			let new_price = document.getElementById(
				`item_number${prod_id}`
			).value;
			...
			let res = await fetch(
				`https://mock-api-4u7p.onrender.com/products/${prod_id}`,
				{
					method: "PATCH",

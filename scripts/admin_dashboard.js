...
if(localStorage.getItem("admin_stay_logged") == "false"){
  ...
  async function getDataHeroku() {
    try {
      let res = await fetch("https://damp-journey-38779.herokuapp.com/products?category=dress&category=clothing");
  ...
  async function deleteProduct(element){
      ...
      try{
          let res = await fetch(`https://damp-journey-38779.herokuapp.com/products/${deleteID}`, {
              method: "DELETE",
              headers:{
                  "Content-Type": "application/json"
              }
  ...
  async function updatePrice(element){
    try{
      ...
      let res = await fetch(`https://damp-journey-38779.herokuapp.com/products/${prod_id}`, {
          method: "PATCH",
